...
def plot_radar_draw_save(scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    #legend_loc = plot_opts.get('legend_loc', 'lower left')
    ...
    for id, id_sort in enumerate(index_sort):
        ...
        if disp_name in ['Artrack-384']:
            all_score_per_tracker[id_sort]=73.1
        if disp_name in ['Artrack-256']:
            all_score_per_tracker[id_sort]=71.0
        if disp_name in ['OTETrack-256']:
            all_score_per_tracker[id_sort]=73.9
        if len(disp_name) > 3 and disp_name[:3] == r'\bf':
